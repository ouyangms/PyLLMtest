...
class PerformanceTester:
    ...
    def test_router_stage(self, num_samples: int = None) -> TestResult:
        ...
        with torch.no_grad():
            for i in range(0, len(test_data), batch_size):
                ...
                indices_tensor = torch.tensor(indices_list, dtype=torch.long).to(self.device)
                ...
                # 统计
                for true_label, pred_label in zip(labels, predicted.tolist()):
        ...
        for cat, stats in category_stats.items():
            if stats["total"] > 0:
                per_category_stats[cat] = {
                    ...
                    "precision": stats["correct"] / stats["total"],  # 简化计算
                    "recall": stats["correct"] / stats["total"],
                    "f1": stats["correct"] / stats["total"],
                    "samples": stats["total"]
                }

        # 总体precision/recall/F1（简化计算，这里等于accuracy）
        precision = accuracy
        recall = accuracy
        f1 = accuracy
    ...
    def _generate_summary_report(self, results: List[TestResult]):
        ...
        print(f"{'阶段':<15} {'准确率目标':<12} {'实际':<10} {'状态':<8}")
        print("-" * 50)
        ...
        for result in results:
            if result.stage in targets:
                ...
                acc_status = "✓" if acc_met else "✗"
                lat_status = "✓" if lat_met else "✗"
