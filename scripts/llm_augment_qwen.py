...
DIMENSION_TEMPLATES = {
    "standard": {
        ...
        "prompt": "生成{count}个标准、清晰的{category}指令，要求：\n"
                 ...
                 "打开主驾驶座椅加热\n"
                 "将空调温度设置为24度\n\n"
                 "请直接生成{count}个{category}的标准指令（每行一个，不要编号）："
    },
    "scenario": {
        ...
        "prompt": "生成{count}个描述场景或感受的{category}指令，要求：\n"
                 ...
                 "有点看不清路\n"
                 "腰有点酸\n\n"
                 "请直接生成{count}个{category}的场景感受型指令（每行一个，不要编号）："
    },
    "colloquial": {
        ...
        "prompt": "生成{count}个口语化、简洁的{category}指令，要求：\n"
                 ...
                 "太热了\n"
                 "调高点\n\n"
                 "请直接生成{count}个{category}的口语化指令（每行一个，不要编号）："
    },
    "negative": {
        ...
        "prompt": "生成{count}个反向操作的{category}指令，要求：\n"
                 ...
                 "停止加热\n"
                 "别太亮\n\n"
                 "请直接生成{count}个{category}的反向操作指令（每行一个，不要编号）："
    },
    "contextual": {
        ...
        "prompt": "生成{count}个复杂或上下文相关的{category}指令，要求：\n"
                 ...
                 "后排窗户都开条缝，透气就行\n"
                 "左边镜子往上抬，我要看后面\n\n"
                 "请直接生成{count}个{category}的复杂指令（每行一个，不要编号）："
    }
}
...
def call_qwen_api(prompt: str, api_key: str, model: str = "qwen-turbo") -> str:
    """调用千问 API"""
    if not HAS_REQUESTS:
    ...
    payload = {
        ...
        "input": {
            "messages": [
                {"role": "system", "content": "你是一个车载语音助手训练数据生成专家。你需要生成各种类型的中文车控指令，要求自然、多样、符合真实用户习惯。只输出指令列表，每行一个，不要编号。"},
...
def generate_samples_qwen(category: str, dimension: str, count: int, api_key: str, model: str = "qwen-turbo") -> List[str]:
    """使用千问 API 生成样本"""
    ...
    try:
        content = call_qwen_api(prompt, api_key, model)
        ...
        for line in content.split('\n'):
            ...
            if line and len(line) >= 2 and len(line) <= 30:
                # 排除明显的解释文字
                if not line.startswith(("注意", "以下是", "示例", "指令", "生成", "好的", "当然", "没问题")):
                    samples.append(line)
...
def generate_samples_openai(category: str, dimension: str, count: int, client) -> List[str]:
    """使用 OpenAI API 生成样本"""
    prompt = DIMENSION_TEMPLATES[dimension]["prompt"].format(
        count=count,
        description=CATEGORY_DESCRIPTIONS[category],
...
def augment_category(
    ...
    for dimension, template in DIMENSION_TEMPLATES.items():
        ...
        if llm_provider == "qwen" and api_key:
            samples = generate_samples_qwen(category, dimension, samples_per_dimension, api_key)
        ...
        # 避免限流
        if llm_provider in ["qwen", "openai"]:
            time.sleep(0.5)
...
def main():
    ...
    parser.add_argument("--samples-per-dimension", type=int, default=20, help="每个维度生成的样本数")
    parser.add_argument("--llm-provider", type=str, default="local", choices=["local", "qwen", "openai"], help="LLM 提供商")
    parser.add_argument("--api-key", type=str, default=None, help="API 密钥")
    parser.add_argument("--category", type=str, default=None, help="指定类别（默认处理所有类别）")
    parser.add_argument("--output", type=str, default="data/processed/augmented_samples.json", help="输出文件")
    parser.add_argument("--qwen-model", type=str, default="qwen-turbo", help="千问模型名称")
    ...
    if args.llm_provider in ["qwen", "openai"] and not api_key:
        print(f"[错误] 使用 {args.llm_provider} 需要提供 API Key")
        print(f"  环境变量: {'DASHSCOPE_API_KEY' if args.llm_provider == 'qwen' else 'OPENAI_API_KEY'}")
    ...
    if args.llm_provider != "local":
        print(f"模型: {args.qwen_model if args.llm_provider == 'qwen' else 'gpt-3.5-turbo'}")
    ...
    for category in categories:
        samples = augment_category(category, args.samples_per_dimension, args.llm_provider, api_key)
        ...
        for sample in samples:
            augmented_data.append({
                ...
                "category": category,
                "source": f"llm_augment_{args.llm_provider}"
            })
